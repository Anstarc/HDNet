...
from .. import backbone as resnet
from ..backbone import resnetc, create_model
from ..utils import batch_pix_accuracy, batch_intersection_union

up_kwargs = {'mode': 'bilinear', 'align_corners': True}

__all__ = ['BaseNet', 'MultiEvalModule', 'SegmentationMultiNLLLosses', 'SegmentationMultiKLLosses', 'SegmentationLovaszLosses', 'SegmentationMultiLosses']

class SegmentationMultiLosses(nn.CrossEntropyLoss):
    """2D Cross Entropy Loss with Multi-Loss"""
    def __init__(self, nclass=-1, weight=None, size_average=True, ignore_index=-1, ohem_ratio=0.0):
        if ohem_ratio == 0.0:
            reduce = True
        else:
            size_average = False
    ...
    def forward(self, preds, target):
        ...
        for p in preds:
            if self.ohem_ratio == 0.0:
                ...
                l = l * mask
                l = l.sum() / (mask.sum() + self.eps)
            loss.append(l.reshape(1,1))
        return torch.cat(loss, 1)

class SegmentationMultiNLLLosses(nn.NLLLoss):
    """2D Cross Entropy Loss with Multi-Loss"""
    def __init__(self, nclass=-1, weight=None, size_average=True, ignore_index=-1, ohem_ratio=0.0):
        if ohem_ratio == 0.0:
            reduce = True
            ...
            size_average = False
            reduce = False
        super(SegmentationMultiNLLLosses, self).__init__(weight, size_average, ignore_index, reduce=reduce)
        self.nclass = nclass
        self.oim = None
        self.ohem_ratio = ohem_ratio
        self.eps = 1e-7
    ...
    def forward(self, preds, target):
        ...
        for p in preds:
            if self.ohem_ratio == 0.0:
                l = super(SegmentationMultiNLLLosses, self).forward(p.log(), target)
            else:
                l = super(SegmentationMultiNLLLosses, self).forward(p.log(), target)
                mask = self._ohem_mask(l, self.ohem_ratio)
                l = l * mask
...
class SegmentationMultiKLLosses(nn.KLDivLoss):
    """2D Cross Entropy Loss with Multi-Loss"""
    def __init__(self, nclass=-1, weight=None, size_average=True, ignore_index=-1):
        super(SegmentationMultiKLLosses, self).__init__()
        self.nclass = nclass
        self.eps = 1e-7
    ...
    def forward(self, preds, target):
        loss = []
        for p in preds:
            l = super(SegmentationMultiKLLosses, self).forward((F.softmax(p,dim=1)).log(), F.softmax(target,dim=1))
            loss.append(l.reshape(1,1))
        return torch.cat(loss, 1)
...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, dilated=True, norm_layer=None,
                 ...
                 multi_grid=False, multi_dilation=None, stride=8):
        ...
        if backbone == 'resnet50':
            self.pretrained = resnet.resnet50(pretrained=True, dilated=dilated,
                                              ...
                                              multi_grid=multi_grid, multi_dilation=multi_dilation, stride=stride)
            ...
            self.pretrained = resnet.resnet152(pretrained=True, dilated=dilated,
                                               norm_layer=norm_layer, root=root,
                                               multi_grid=multi_grid, multi_dilation=multi_dilation, stride=stride)
        elif backbone == 'resnet101c':
            self.pretrained = resnetc.resnet101(pretrained=True, dilated=dilated,
                                               norm_layer=norm_layer, root=root,
                                               multi_grid=multi_grid,multi_dilation=multi_dilation, stride=stride)
        elif 'efficient' in backbone or 'resnest' in backbone or 'mobilenet' in backbone:
            self.pretrained = create_model(backbone, pretrained=True)
        elif backbone == 'None':
        ...
        self.criterion = SegmentationMultiLosses(nclass=nclass)

    def base_forward(self, x, stage0=False):
        if 'efficient' in self.backbone or 'mobilenetv3' in self.backbone:
            return self.pretrained(x)
        elif 'resnest' in self.backbone:
            c0, c1, c2, c3, c4 =  self.pretrained.forward_features(x)
