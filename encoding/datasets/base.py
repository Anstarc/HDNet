...
class BaseDataset(data.Dataset):
    ...
    def _sync_transform(self, img, mask):
        ...
        if self.base_size != 0:
            ...
            scale = random.randint(2,8) * 0.25
            w, h = img.size
            oh = int(scale * h)
            ow = int(scale * w)

            short_size = min(oh, ow)
        ...
        if short_size < crop_size:
            ...
            mask = ImageOps.expand(mask, border=(0, 0, padw, padh), fill=self.pad)#pad 255 for cityscapes
    ...
    def _mask_transform(self, mask):
        return torch.from_numpy(encode_segmap(np.array(mask))).long()
