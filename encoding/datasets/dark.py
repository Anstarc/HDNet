...
class DarkSegmentation(BaseDataset):
    NUM_CLASS = 4   # 0:non-skin, 1:face, 2:body, 3:non-skin in face
    BASE_DIR = 'dark'
    def __init__(self, root, split='train', mode=None, transform=None, 
                 target_transform=None, **kwargs):
        super(DarkSegmentation, self).__init__(root, split, mode, transform,
        ...
        with open(os.path.join(_split_f), "r") as lines:
            for line in tqdm(lines):
                ...
                self.val_names.append(line.rstrip('\n'))
                if self.mode != 'test':
