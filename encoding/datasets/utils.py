...
def decode_segmap(label_mask, dataset, plot=False, div255=True, unlabeled=False):
    ...
    """
    if dataset == 'pascal_voc' or dataset == 'coco':
        n_classes = 21
        label_colours = get_pascal_labels()
    elif dataset == 'cityscapes':
        n_classes = 19
        label_colours = get_cityscapes_labels()
    elif 'pcontext' in dataset:
        n_classes = 59
        label_colours = get_pascal_context_labels()[:59]
    elif 'ecu' in dataset:
        n_classes = 2
        label_colours = np.array([[0,0,0], [255,255,255]])
        n_classes = 4
        label_colours = np.array([[0, 0, 0], [128, 0, 0], [0, 128, 0], [128, 128, 0]])
    elif 'dark' in dataset:
        n_classes = 4
        label_colours = np.array([[0, 0, 0], [128, 0, 0], [0, 128, 0], [128, 128, 0]])
    else:
    ...
    if unlabeled:
        n_classes += 1
        unlabel_colours = np.array([[0,0,0]])
        label_colours = np.vstack((unlabel_colours, label_colours))
...
def get_pascal_context_labels():

    return np.array([[128, 0, 0], [0, 128, 0], [128, 128, 0], [0, 0, 128], [128, 0, 128],
