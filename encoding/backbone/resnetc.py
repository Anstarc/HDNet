...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=False, stride=8, multi_grid=False,
                 deep_base=True, norm_layer=nn.BatchNorm2d, multi_dilation=None):
        ...
        if dilated:
            self.layer3 = self._make_layer(block, 256, layers[2], stride=stride//8,
                                           dilation=16//stride, norm_layer=norm_layer)
            if multi_grid:
                self.layer4 = self._make_layer(block, 512, layers[3], stride=1,
                                               dilation=32//stride, norm_layer=norm_layer,
                                               multi_grid=True)
            else:
                self.layer4 = self._make_layer(block, 512, layers[3], stride=1,
                                               dilation=32//stride, norm_layer=norm_layer)
